...
import { lookupDeliveryAvailability, type DeliveryCheckResult } from './shipping.js';
import {
  addDays,
  asMoney,
...
export async function loadCoupon(sql: NeonQueryFunction<false, false>, code?: string): Promise<CouponRow | null> {
  if (!code) return null;
...
export type CheckoutPurchasePrep = {
  item: CatalogItemRow;
  quantity: number;
  coupon: CouponRow | null;
  subtotal: number;
  ...
  total: number;
  courseAccess: CourseAccessRow | null;
  delivery: DeliveryCheckResult | null;
};
...
export async function prepareCheckoutPurchase(
  ...
  user: SessionUser,
  body: CheckoutBody
): Promise<{ ok: false; status: number; error: string } | { ok: true; prep: CheckoutPurchasePrep }> {
  const slug = typeof body.product === 'string' ? body.product.trim() : '';
  if (!slug) {
    return { ok: false, status: 400, error: 'Product is required.' };
  }
  ...
  if (item.type === 'physical_book' && item.stock_quantity !== null && item.stock_quantity < quantity) {
    return { ok: false, status: 400, error: 'This book is currently out of stock.' };
  }
  if (item.type === 'live_class') {
    if (getLiveClassStatus(item.metadata) === 'cancelled') {
      return { ok: false, status: 400, error: 'This live class has been cancelled and can no longer be purchased.' };
    }
    if (item.spots_remaining !== null && item.spots_remaining <= 0) {
      return { ok: false, status: 400, error: 'This live class is sold out.' };
    }
  }
  ...
  if (item.type === 'physical_book') {
    ...
    if (
      ...
      return { ok: false, status: 400, error: 'Shipping details are required for physical books.' };
    }
    const delivery = await lookupDeliveryAvailability(sql, shipping.pinCode.trim());
    if (!delivery.available) {
      return { ok: false, status: 400, error: delivery.message };
    }
  }
...
export async function commitCheckoutPurchase(
  sql: NeonQueryFunction<false, false>,
  user: SessionUser,
  body: CheckoutBody,
  prep: CheckoutPurchasePrep,
  orderNumber: string,
  ...
  orderNumber: string;
  total: number;
  item: ReturnType<typeof mapCatalogItem>;
  unlocked: boolean;
}> {
  const { item, quantity, coupon, subtotal, discount, total, courseAccess, delivery } = prep;
